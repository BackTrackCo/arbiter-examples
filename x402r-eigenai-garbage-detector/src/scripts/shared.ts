...
import { BASE_SEPOLIA_RPC, CHAIN_ID, CONTEXT_FILE, EIGENAI } from "../config.js";
...
export function createEigenAI() {
  const { account } = createClients();
  return new EigenAIClient(account, EIGENAI.grantServer, EIGENAI.model);
}
